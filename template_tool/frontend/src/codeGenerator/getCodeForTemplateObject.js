...
const spanPaths = [
  "block_type",
  "location.steps",
  "location.has_measure",
  "repeat.repeat_count",
  "filters.has_name",
  "filters.has_size",
  "filters.has_colour",
  "num_blocks.block_filters.has_name",
  "num_blocks.block_filters.has_size",
  "num_blocks.block_filters.has_colour",
  "stop_condition.block_type",
  "repeat.repeat_count",
];
...
function getCodeForTemplateObject(surfaceForm, topBlock) {
  ...
  var code;
  if (codeString && codeString != "undefined") {
  ...
  if (code?.ordinal) {
    ...
    if (span == "first" || span == "second" || span == "third") {
    ...
    categoricalPaths.forEach((path) => {
      if (nestedProperty.get(code, path)) {
        nestedProperty.set(code, path, span.toUpperCase());
      }
    });
  }
